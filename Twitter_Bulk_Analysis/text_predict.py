...
from Reporting_Platform.pybert.configs.basic_config import config
...
class BertProcessor(object):
    ...
    def __init__(self,vocab_path,do_lower_case):
        # Use from_pretrained to automatically handle missing local vocab files
        try:
            self.tokenizer = BertTokenizer.from_pretrained("bert-base-multilingual-uncased", do_lower_case=do_lower_case)
        except:
...
def predict_string(input_string):
    try:
        ...
        # If model loading fails, return dummy scores as requested by user
        import random
        # Classes: ["toxic","severe_toxic","obscene","threat","insult","identity_hate"]
        # Return mostly low scores, but occasional random spikes to see the UI reaction
        return [random.uniform(0, 0.2) for _ in range(6)]
    # classes = ["toxic","severe_toxic","obscene","threat","insult","identity_hate"]
    # print("Text:",text)
